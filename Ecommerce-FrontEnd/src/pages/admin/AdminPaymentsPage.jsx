  ...
  const fetchPayments = async () => {
    ...
    try {
      const res = await axiosInstance.get("/admin/payments", {
        params: { page, pageSize: 10, search, status: statusFilter, method: methodFilter },
      });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap" }}>
        <input className="form-input" placeholder="Search by payment ID or customer email..." value={search}
          ...
          <tbody>
            {payments.map(p => (
              <tr key={p.paymentId}>
                ...
                <td>{p.orderId?.slice(0,8).toUpperCase()}</td>
                <td>{p.method}</td>
