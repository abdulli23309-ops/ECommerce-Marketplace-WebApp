...
const PermissionGroupsPage = () => {
  ...
  const fetchAllPermissions = async () => {
    try {
      const res = await axiosInstance.get("/admin/permissions/all");
  ...
  const handleDelete = async (groupId) => {
    ...
    try {
      ...
      fetchGroups();
    } catch (err) { alert(err.response?.data?.message || "Cannot delete group."); }
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button className="add-product-btn" onClick={openCreateModal}>+ Add Group</button>
      </div>
            ...
            <tbody>
              {groups.map(group => (
                <tr key={group.id}>
                  ...
                  <td>
                    <button className="btn-edit" onClick={() => openEditModal(group)}>Edit</button>
              ...
              <div key={module} style={{ marginBottom: "1rem" }}>
                ...
                {perms.map(perm => (
                  <label key={perm.id} style={{ display: "block", marginLeft: "1rem", fontSize: "0.9rem", cursor: "pointer" }}>
                    <input type="checkbox" checked={selectedPermIds.includes(perm.id)}
                      onChange={() => handleCheckboxChange(perm.id)} style={{ marginRight: "0.5rem" }} />
                    {perm.name} <span style={{ color: "#888", fontSize: "0.8rem" }}>({perm.code})</span>
                  </label>
                ))}
              </div>
            ))}
